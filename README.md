...
- `*`：**本机通配**——含义是"把我所属机器人的所有会话都算进这个组"
- `qq-bot:*`：指定机器人的全部会话

#### `*` 的准确语义（容易误解，注意）

- `*` 让**任何机器人**的会话都可以成为该组的成员
- 但每个会话**展开后只包含"自己所属机器人"的全部会话**——它不会把其他机器人的会话卷进来
- 因此组 `["*", "wechat-bot:FriendMessage:10001"]` 的实际效果是：**本机所有会话 ↔ 微信那一个会话（单向桥）**。本机能看见微信，但微信侧展开的 `*` 是它自己的机器人，看不到本机

要让跨机器人**双向**互通，需要把对端机器人也显式声明进组：
...
此时微信侧的 `qq-bot:*` 条目对它同样生效，微信能看到 qq-bot 的全部会话，双向对等。
